...
This one is really an intro, we see a page "Fetch Data", click on it and it is possible to fetch internal files. Just click the button and get the flag :
...
![image](/assets/img/wanictf24/Pasted image 20240623130108.png)
...
We try to inject XSS payload in the `profile` field, however, the CSP rules prevent from actually executing code : ![[csp.png]]
...
Additionally, it does not implement any CSP ! The `Content-Type` of this page is `text/plain` when we attempt to access it directly, but upon redirection, the content will be served as `text/html` to the end-user.
