...
The binary is 64-bits stripped ELF named `hidden.bin`.

To begin the investigation, I lauched the automatics analysis with radare2
and the decompile the function identified as the `main`.
...
I simply set a breakpoint at this `call` instructions,
and when I jumped in the function, GDB printed a name and path of dynamically-linked libray
used by `hidden.bin`.
...
To do it, I export the CFG representation of this function
in GraphViz DOT before generate a PNG file.
...
Firstly we can identify a `switch...case` structure implemented as ``if``-cascade by the compiler.

![switch case cfg](/assets/img/lehack2025/switch_case_cfg.png)
